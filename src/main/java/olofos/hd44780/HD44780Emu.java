package olofos.hd44780;

import java.util.Arrays;

public class HD44780Emu {
	public enum Rom {
		A00, // Japanese
		A02 // European
	}

	static public final int COLOR_BG = -1;
	static public final int COLOR_OFF = 0;
	static public final int COLOR_ON = +1;
	...
	static final int DATA_WIDTH_CHARS_1ROW = 80;
	static final int DATA_WIDTH_CHARS_2ROW = 40;
	static final int DATA_WIDTH_CHARS_4ROW = 20;
	static final int DDRAM_SIZE = 128;
	static final int DDRAM_VISIBLE_SIZE = 80;

	static final int DISPLAY_MIN_COLS = 8;
	static final int DISPLAY_MAX_COLS = 40;
	static final int DISPLAY_MIN_ROWS = 1;
	static final int DISPLAY_MAX_ROWS = 4;

	static final int CGRAM_STORAGE_CHARS = 16;
	static final int ROM_FONT_CHARS = (256 - CGRAM_STORAGE_CHARS);
	static final int CGRAM_SIZE = (CGRAM_STORAGE_CHARS * CHAR_HEIGHT_PX);

	static final int CURSOR_MASK = (LCD_CMD_DISPLAY_CURSOR_BLINK | LCD_CMD_DISPLAY_CURSOR);
	static final int CURSOR_BLINK_PERIOD_MS = 350;
	static final int CURSOR_BLINK_CYCLE_MS = (2 * CURSOR_BLINK_PERIOD_MS);
	...
	private final int dataWidthCols;

	private int functionFlags;
	private int entryModeFlags;
	private int displayFlags;
	private int scrollOffset;

	private AddressMode addressMode;

	private final int[] ddRam;
	private int ddIndex;
	...
	public HD44780Emu(int cols, int rows, Rom rom) {
		...
		addressMode = AddressMode.DDRAM;

		ddRam = new int[DDRAM_SIZE];
		ddIndex = 0;

		cgRam = new int[CGRAM_STORAGE_CHARS][CHAR_WIDTH_PX];
		cgIndex = 0;

		entryModeFlags = LCD_CMD_ENTRY_MODE_INCREMENT;
		functionFlags = LCD_CMD_FUNCTION_DL;

		upperBitsR = -1;
		lowerBitsW = -1;

		width = cols * (CHAR_WIDTH_PX + 1) - 1;
		height = rows * (CHAR_HEIGHT_PX + 1) - 1;
		...
		if (rows == 1) {
			dataWidthCols = DATA_WIDTH_CHARS_1ROW;
		} else if (rows == 2) {
			dataWidthCols = DATA_WIDTH_CHARS_2ROW;
		} else {
			dataWidthCols = DATA_WIDTH_CHARS_4ROW;
		}
	...
	public void sendCommand(int command) {
		if ((functionFlags & LCD_CMD_FUNCTION_DL) == 0) {
			if (upperBitsR < 0) {
				upperBitsR = command & 0xF0;
				return;
			}
			command = upperBitsR | ((command >> 4) & 0x0F);
			upperBitsR = -1;
		}

		if ((command & LCD_CMD_SET_DDRAM_ADDR) != 0) {
			ddIndex = command & 0x7F;
			addressMode = AddressMode.DDRAM;
		} else if ((command & LCD_CMD_SET_CGRAM_ADDR) != 0) {
			cgIndex = command & 0x3F;
			addressMode = AddressMode.CGRAM;
		} else if ((command & LCD_CMD_FUNCTION) != 0) {
			functionFlags = command;
		} else if ((command & LCD_CMD_SHIFT) != 0) {
			if ((command & LCD_CMD_SHIFT_DISPLAY) != 0) {
				if ((command & LCD_CMD_SHIFT_RIGHT) != 0) {
					scrollOffset -= 1;
				} else {
					scrollOffset += 1;
				}
		...
		} else if ((command & LCD_CMD_DISPLAY) != 0) {
			displayFlags = command;
		} else if ((command & LCD_CMD_ENTRY_MODE) != 0) {
			entryModeFlags = command;
		} else if ((command & LCD_CMD_HOME) != 0) {
			ddIndex = 0;
			scrollOffset = 0;
		} else if ((command & LCD_CMD_CLEAR) != 0) {
			Arrays.fill(ddRam, 0);
			ddIndex = 0;
			scrollOffset = 0;
		}
	...
	public void writeByte(int data) {
		if ((functionFlags & LCD_CMD_FUNCTION_DL) == 0) {
			if (upperBitsR < 0) {
				upperBitsR = data & 0xF0;
				return;
			}
			data = upperBitsR | ((data >> 4) & 0x0F);
			upperBitsR = -1;
		}

		if (addressMode == AddressMode.CGRAM) {
			// Find row offset
			int y = cgIndex % CHAR_HEIGHT_PX;
			int[] cgEntry = cgRam[(cgIndex - y) / CHAR_HEIGHT_PX];
			for (int i = 0; i < CHAR_WIDTH_PX; i++) {
				boolean bit = (data & ((1 << (CHAR_WIDTH_PX - 1)) >> i)) != 0;
				if (bit) {
					cgEntry[i] |= (0x80 >> y) & 0xFF;
				} else {
					cgEntry[i] &= ~(0x80 >> y) & 0xFF;
				}
			}
	...
	private int[] charBits(int c) {
		if (c < CGRAM_STORAGE_CHARS) {
			return cgRam[c];
		}
		int index = c - CGRAM_STORAGE_CHARS;
		if (rom == Rom.A00) {
			return fontA00[index];
		}
		return fontA02[index];
	}

	private int getDataOffset(int row, int col) {
		// For information about addressing the LCD see
		// https://web.alfredstate.edu/faculty/weimandn/lcd/lcd_addressing/lcd_addressing_index.html
		int rowOffsets[] = { 0x00, 0x40, 0x00 + cols, 0x40 + cols };

		if (row > rows) {
			row = rows - 1;
		}
		int dataCol = (col + scrollOffset) % dataWidthCols;
		return rowOffsets[row] + dataCol;
	}

	private void updatePixels() {
		int cursorOn = (displayFlags & CURSOR_MASK);

		boolean displayOn = (displayFlags & LCD_CMD_DISPLAY_ON) != 0;

		if (System.currentTimeMillis() % CURSOR_BLINK_CYCLE_MS < CURSOR_BLINK_PERIOD_MS) {
			cursorOn &= ~LCD_CMD_DISPLAY_CURSOR_BLINK;
		}

		for (int row = 0; row < rows; row++) {
			for (int col = 0; col < cols; col++) {
				int index = getDataOffset(row, col);
				boolean drawCursor = (cursorOn != 0) && (index == ddIndex);
				int[] bits = charBits(ddRam[index]);
				for (int y = 0; y < CHAR_HEIGHT_PX; y++) {
					for (int x = 0; x < CHAR_WIDTH_PX; x++) {
						int pixel = 1;
						if (displayOn) {
							pixel = ((bits[x] & (0x80 >> y)) != 0) ? 1 : 0;
							if (drawCursor) {
								if (((cursorOn & LCD_CMD_DISPLAY_CURSOR) != 0) && (y == CHAR_HEIGHT_PX - 1)) {
									pixel = 1;
								}
								if ((cursorOn & LCD_CMD_DISPLAY_CURSOR_BLINK) != 0) {
									pixel = 1;
								}
							}
						}
						pixels[col * (CHAR_WIDTH_PX + 1) + x][row * (CHAR_HEIGHT_PX + 1) + y] = pixel;
					}
				}
			}
		}
	...
	private void increment() {
		...
		if (rows > 1) {
			if (ddIndex == 0x28) {
				ddIndex = 0x40;
			} else if (ddIndex == 0x68 || ddIndex >= DDRAM_SIZE) {
				ddIndex = 0;
			}
		} else if (ddIndex >= DDRAM_VISIBLE_SIZE) {
			ddIndex = 0;
		}
	}
	...
	private void decrement() {
		...
		if (rows > 1) {
			if (ddIndex == -1) {
				ddIndex = 0x67;
			} else if (ddIndex == 0x39) {
				ddIndex = 0x27;
			}
		}
		if (ddIndex == -1) {
			ddIndex += DDRAM_VISIBLE_SIZE;
		} else if (ddIndex >= DDRAM_SIZE) {
			ddIndex = 0;
		}
	}
	...
	private void shift() {
		if (addressMode == AddressMode.CGRAM) {
			if ((entryModeFlags & LCD_CMD_ENTRY_MODE_INCREMENT) != 0) {
				cgIndex += 1;
				if (cgIndex >= CGRAM_SIZE) {
					cgIndex = 0;
				}
				return;
			}
			...
			cgIndex -= 1;
			if (cgIndex < 0) {
				cgIndex = CGRAM_SIZE - 1;
			}
		}

		if ((entryModeFlags & LCD_CMD_ENTRY_MODE_SHIFT) != 0) {
			if ((entryModeFlags & LCD_CMD_ENTRY_MODE_INCREMENT) != 0) {
				scrollOffset += 1;
			} else {
				scrollOffset -= 1;
